...
namespace webrtc
{
    ...
    public:
        ...
        // Per-frame import: one AHB -> image(+memory) and, for the ycbcr path, a
        // ycbcr-aware view referencing the shared conversion. Caller frees via FreeImage.
        bool ImportImage(AHardwareBuffer* ahb, uint32_t width, uint32_t height, AhbFrameImage* out);
        void FreeImage(AhbFrameImage& img);

        // Release the shared conversion + sampler (call after all frames are freed).
        void Shutdown();
